...
export async function initServicesIndex() {
  const host=document.querySelector('.catalog-page'); if(!host)return;
  const query=new URLSearchParams(location.hash.startsWith('#/') ? location.hash.split('?')[1]||'' : location.search||'');
