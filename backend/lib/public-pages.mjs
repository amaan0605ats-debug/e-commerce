...
export function sitemap(items=catalog()) {
 return '<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'+['/','/about','/services','/contact',...items.map(s=>'/services/'+encodeURIComponent(s.slug))].map(p=>'<url><loc>'+escapeMarkup(SITE_URL+p)+'</loc></url>').join('')+'</urlset>';
}
export function renderPublicPage(template,path,items=catalog()) {
 ...
 const routes={'/':renderHome,'/about':renderAbout,'/services':renderServicesIndex,'/contact':renderContact};
